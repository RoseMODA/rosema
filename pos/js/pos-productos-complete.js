 ...
 * Versión actualizada con múltiples imágenes y modal de confirmación personalizado
 */
...
function showExistingImages(images) {
  const previewContainer = document.getElementById('image-preview-container');
  if (!previewContainer || !images || images.length === 0) return;
  ...
  images.forEach((imageUrl, index) => {
    ...
    previewItem.innerHTML = `
      <div class="aspect-square bg-gray-100 rounded-lg overflow-hidden border-2 border-gray-200">
        <img src="${imageUrl}" alt="Imagen ${index + 1}" class="w-full h-full object-cover" onerror="this.parentElement.innerHTML='<div class=\\'w-full h-full flex items-center justify-center text-gray-400\\'>Error al cargar</div>'">
      </div>
