...
function calculateDashboardStats(products) {
  ...
  products.forEach(product => {
    ...
    categoryStats[category].value += (product.price || 0) * (product.stock || 0);
  });
      ...
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <div class="space-y-4">
          <div class="flex justify-between items-center">
            ...
            <span class="font-semibold">${formatNumber(stats.totalProducts)}</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-gray-600">Stock total:</span>
            <span class="font-semibold">${formatNumber(stats.totalStock)} unidades</span>
          </div>
          <div class="flex justify-between items-center">
            ...
            lowStockProducts.slice(0, 10).map(product => `
              <div class="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                <div class="flex-1">
                  ...
                  <p class="text-xs text-gray-500">${product.sku || 'Sin SKU'}</p>
                </div>
                <div class="text-right">
                  <span class="text-sm font-semibold text-red-600">${product.stock || 0} unidades</span>
                </div>
              </div>
            `).join('') :
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ${Object.entries(stats.categoryStats).map(([category, data]) => `
          <div class="border border-gray-200 rounded-lg p-4">
            <h4 class="font-medium text-gray-900 capitalize mb-2">${category.replace('-', ' ')}</h4>
