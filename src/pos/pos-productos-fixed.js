...
import { debounce, showNotification } from "./pos-utils.js";
...
/**
 * Wrapper para crear un producto usando addProduct
 */
async function createProduct(productData, imageFiles) {
  // TODO: Implementar manejo de imageFiles si es necesario
  try {
  ...
  return `
    ...
    <div class="mb-4">
      <p class="text-sm text-gray-600">
        <span id="products-count">${currentProducts.length}</span> producto${
...
function setupImageUpload() {
  ...
  if (typeof enableDragAndDrop === "function") {
    enableDragAndDrop(dropZone, handleImageDrop, [
      "image/jpeg",
      "image/png",
      "image/webp",
    ]);
...
function processSelectedImages(files) {
  ...
  for (const file of files) {
    if (!validateFileType(file, ["image/jpeg", "image/png", "image/webp"])) {
      showNotification(
        `Formato no válido para ${file.name}. Use JPG, PNG o WebP`,
        "error"
      );
      continue;
    }
    ...
    if (!validateFileSize(file, 5)) {
      showNotification(
        `${file.name} es muy grande. Máximo 5MB por imagen`,
        "error"
      );
      continue;
    }
  ...
  showNotification(
    `${validFiles.length} imagen${
      validFiles.length > 1 ? "es" : ""
    } seleccionada${validFiles.length > 1 ? "s" : ""}`,
    "success"
  );
}
  ...
  tbody.innerHTML = filteredProducts
    ...
    <tr class="hover:bg-gray-50">
      <td class="px-6 py-4 whitespace-nowrap">
        <input type="checkbox" class="product-checkbox rounded border-gray-300" data-id="${
          product.id
        }">
      </td>
      <td class="px-6 py-4 whitespace-nowrap">
        <div class="flex items-center">
          <div class="flex-shrink-0 h-12 w-12">
            ${
              product.images && product.images.length > 0
                ? `<img class="h-12 w-12 rounded-lg object-cover" src="${
                    product.images[0]
                  }" alt="${sanitizeText(
                    product.name
                  )}" onerror="this.parentElement.innerHTML='<div class=\\'h-12 w-12 rounded-lg bg-gray-200 flex items-center justify-center\\'><span class=\\'text-gray-400 text-xs\\'>Sin imagen</span></div>'">`
          ...
          <div class="ml-4">
            ...
            ${
              product.category
                ? `<div class="text-xs text-gray-400">${sanitizeText(
                    product.category
                  )}</div>`
                : ""
            }
      ...
      <td class="px-6 py-4 whitespace-nowrap">
        <div class="text-sm text-gray-900">${formatCurrency(
          product.price || 0
        )}</div>
        ${
          product.originalPrice
            ? `<div class="text-xs text-gray-500 line-through">${formatCurrency(
                product.originalPrice
              )}</div>`
            : ""
        }
      ...
      <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
        <button onclick="editProduct('${
          product.id
        }')" class="text-blue-600 hover:text-blue-900 mr-3" title="Editar producto">
          Editar
        </button>
...
async function handleSaveProduct() {
  try {
    ...
    if (
      ...
      showNotification(
        "Por favor completa todos los campos requeridos: nombre, categoría y precio",
        "error"
      );
      return;
    }
    ...
    for (const file of imageFiles) {
      if (!validateFileType(file)) {
        showNotification(
          `Formato de imagen no válido para ${file.name}. Use JPG, PNG o WebP`,
          "error"
        );
        return;
      }
      if (!validateFileSize(file)) {
        showNotification(
          `El archivo ${file.name} es muy grande. Máximo 5MB por imagen`,
          "error"
        );
        return;
      }
    ...
    if (result && result.success) {
      ...
      showNotification(
        result?.error || "Error desconocido al guardar producto",
        "error"
      );
    }
  } catch (error) {
